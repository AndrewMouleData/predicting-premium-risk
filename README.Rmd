...
Assess whether open UK road safety data can be used to estimate relative risk across vehicle characteristics, and, if so, how and to what extent those estimates can support third-party motor insurance premium-pricing decisions.
...
A key advantage of this structure is that it preserves the distinction between different kinds of high-risk profile. Some profiles may rank highly because they appear frequently in the collision data. Others may appear less often, but generate materially worse injury outcomes when they do. Separating frequency from severity makes that difference visible, which allows for greater clarity in the insights this analysis produces.
...
The business task for this project was to assess whether open UK road safety data can be used to estimate relative risk across vehicle-characteristic profiles, and, if so, how and to what extent those estimates can support third-party motor insurance premium-pricing decisions.

This has been addressed by:
  
* Transforming raw STATS19 casualty, collision, and vehicle records into composite, grouped, thresholded, and interpretable vehicle-characteristic profiles
* Applying a heuristic frequency × severity proxy to explore the relative risk landscape within the STATS19-derived dataset
* Testing whether the strength, coverage, and completeness of the resulting analytical framework were sufficient to support meaningful relative-risk interpretation

These processes were necessary because the business task could not be addressed directly from the raw STATS19 files. To support relative vehicle-characteristic risk assessment, the raw casualty, collision, and vehicle records first had to be transformed into a structured analytical dataset built around grouped profiles. The fact that STATS19 could support this transformation is central to the feasibility element of the task: it demonstrates that open UK road safety data can provide a workable foundation from which relative road risk can be estimated, provided it is processed through a clear and disciplined data analytics pipeline.

The extent element of the business task concerns how far that framework can be interpreted with confidence and used to support premium-pricing decisions. This is answered through the findings themselves: by assessing the risk signals produced, the rankings and aggregations derived from them, and the strength, coverage, and completeness of the evidence underpinning those outputs.

The first four findings show that this open-data-derived analytical framework can produce a meaningful relative-risk landscape, while also beginning to define the conditions under which its outputs should be interpreted. Finding 1 demonstrates that grouped vehicle-characteristic profiles separate according to distinct frequency and severity patterns, allowing risk to be interpreted in different ways: as volume-led, severity-led, elevated across both dimensions, or lower across both dimensions. Finding 2 then shows that these profiles can be ranked under `risk_proxy_score`, producing an ordinal view of which vehicle-characteristic combinations sit highest within the STATS19-derived proxy risk landscape. Finding 3 supports this by showing that the proxy distribution contains enough spread to enable relative comparison. Finding 4 then demonstrates that the framework can also support different levels of analytical granularity, with profile-level signals aggregated into broader grouped vehicle-type contributions where a higher-level portfolio view is desirable. This broader aggregation can help mitigate some of the constraints created by the STATS19 source taxonomy, allowing interpretation to continue even where the original vehicle categories are unevenly defined or insufficiently detailed for more granular analysis. Together, these findings indicate that STATS19 provides the basis for an analytical framework that creates a structured, interpretable view of relative risk pressure across vehicle characteristics, while also acknowledging that interpretation remains tied to the structure and limits of the underlying collision-involved vehicle data.

Findings 5 and 6 examine questions of confidence, coverage, and completeness most directly. Finding 5 shows that profile depth affects confidence in interpretation: even after thresholding, the severity component of the proxy is not supported equally across the reporting subset. Higher-volume profiles are likely to provide more stable average severity signals, while lower-count profiles may be more influenced by a small number of serious or fatal outcomes. Finding 6 then extends this assessment from profile depth to analytical coverage, showing how the final reporting subset is shaped by the trade-off between stability, interpretability, and completeness. Thresholding removes many lower-representation profiles while preserving most collision-involved vehicles, but the reporting-level undefined-profile filter removes a meaningful share of threshold-surviving vehicles that lack sufficient characteristic detail. Together, these findings show that confidence is not uniform across the analytical framework: it is strongest for better-represented and better-defined vehicle-characteristic profiles, while more niche, lower-volume, or poorly described profiles should be treated as directional indicators requiring further validation.

Drawing a conclusion from these findings, the answer to the business task is positive, but conditional. In binary terms, STATS19 satisfies the feasibility test within the business task: it can be used for the purpose outlined in the task. It contains sufficient signal to differentiate relative risk across vehicle-characteristic profiles, and it can be transformed into an analytical framework capable of supporting structured risk assessment. As evidenced by this project, open UK road safety data can be used for this purpose by converting raw records into structured analytical profiles, applying a transparent frequency × severity proxy, and interpreting the resulting outputs as a comparative risk landscape.

The more qualified part of the answer concerns the *extent* to which this framework can support premium-pricing decisions. Within the scope of this project, STATS19 is most useful at the risk-signalling and prioritisation stage of that process. It can help identify which vehicle-characteristic profiles appear to carry greater relative risk pressure, which areas may warrant closer pricing review, and where insurer-held claims, policy, exposure, and vehicle population data should be brought in before any monetary pricing decisions are made. These constraints are consistent with the project’s starting premise: STATS19 can support a heuristic risk-profiling framework, but it does not contain the actuarial, exposure, policy, or claims data required for direct premium setting. The estimates produced here are therefore best understood as broad, comparative risk indicators rather than actuarial pricing outputs. Their usefulness is strongest for better-represented and better-defined profiles, while niche, lower-volume, or poorly described segments should be treated as directional signals rather than pricing-ready conclusions.